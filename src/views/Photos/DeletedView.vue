...
export default defineComponent({
  ...
  methods: {
    ...
    deletePhotos(ids: string[]) {
      this.$modal
        .show(ConfirmDialog, {
          props: {
            title: `Delete ${ids.length} photo${
              ids.length > 1 ? 's' : ''
            } permanently?`,
            message: `Are you sure you want to delete ${ids.length} photo${
              ids.length > 1 ? 's' : ''
  ...
  computed: {
    photos(): Photo[] {
      return Photo.query()
        .where('deleted_at', (val: any) => val != null)
